...
<!--    {{ stat }}-->
    <svg :height="height * 2" width="100%">
      ...
      <text :x="y0 + 10" :y="height / 2" dy="0.4em" v-if="y0 > 0">{{this.stat.fmt(this.stat.Y0)}}</text>
      <rect x="0" :y="height" :width="y1" :height="height" fill="#1114aa" opacity="0.6" />
      <text :x="y1 + 10" :y="height * 3 / 2" dy="0.4em" v-if="y1 > 0">{{this.stat.fmt(this.stat.Y1)}}</text>
    </svg>
  </div>
</template>
